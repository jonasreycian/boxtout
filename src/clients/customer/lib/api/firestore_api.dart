...
class FirestoreApi {
  ...
  Future<List<Address>> getAddressListForUser(String userId) async {
    ...
    try {
      ...
      List<Address> addresses = addressCollection.docs.map((address) {
        return Address.fromJson(address.data() as Map<String, dynamic>);
      }).toList();
      return addresses;
    } catch (error) {
      throw FirestoreApiException(
  ...
  String extractRegionIdFromUserAddresses({required List<Address> addresses, required String userDefaultAddressId}) {
    ...
    try {
      ...
      throw FirestoreApiException(
        devDetails: error.toString(),
        message: "we couldn't found the default address of the user in our address collection",
      );
    }
  }

  Future<List<Merchant>> getMerchantsCollectionForRegion({required String regionId}) async {
    log.i('regionId:$regionId');
    try {
      final regionCollections = await regionsCollection.doc(regionId).collection(MerchantsFirestoreKey).get();
      ...
      log.v('for regionId: $regionId, Merchants fetched: $regionCollectionsDocuments');
      ...
      throw FirestoreApiException(devDetails: error.toString(), message: 'An error ocurred while calling getMerchantsCollectionForRegion()');
    }
  }
}
